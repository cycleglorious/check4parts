...
self.onmessage = async (event) => {
	...
	if (!Number.isInteger(startFrom) || startFrom < 0) {
		self.postMessage({
			type: 'error',
			error: "Невірний параметр startFrom: має бути невід'ємним цілим числом."
		});
		return;
	}
	if (!Number.isInteger(previewRowsCount) || previewRowsCount < 0) {
		self.postMessage({
			type: 'error',
			error: "Невірний параметр previewRowsCount: має бути невід'ємним цілим числом."
		});
		return;
	}
	if (!Number.isInteger(startPreviewFrom) || startPreviewFrom < 0) {
		self.postMessage({
			type: 'error',
			error: "Невірний параметр startPreviewFrom: має бути невід'ємним цілим числом."
		});
		return;
	}
	...
	try {
		...
		self.postMessage({
			type: 'progress',
			message: 'Генерація попереднього перегляду...',
			percentage: 70
		});
		...
		self.postMessage({
			type: 'preview',
			previewData,
			metadata: { headers, rowCount: previewRows.length }
		});
		...
		self.postMessage({
			type: 'error',
			error: `Помилка при обробці файлу: ${err.message || 'Невідома помилка'}`
		});
	}
};
