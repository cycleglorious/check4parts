...
async function uploadPricesToSupabaseInWorker(
  ...
  try {
    ...
    if (historyError) {
      throw new Error(`Помилка створення історії цін: ${historyError.message}, ${JSON.stringify(historyError)}, ${providerId}`);
    }
    ...
    for (let i = 0; i < totalCount; i += chunkSize) {
      ...
      if (chunkToProcess.length > 0) {
        allChunkPromises.push(
          (async (currentChunk, currentChunkIndex) => { // Wrap in an IIFE to capture chunk context
    ...
    if (historyId) {
      postMessageToMain({ type: 'progress', payload: { uploadedCount: totalCount, totalCount, percentage: 95, message: 'Завершення завантаження...' } });
      const { error: updateError } = await supabase.from('price_history').update({ status: 'actual' }).eq('id', historyId);
      if (updateError) {
        console.warn(`Помилка оновлення статусу історії цін (${historyId}): ${updateError.message}`);
      }
    }
    ...
    if (historyId && historyCreatedAt) {
      postMessageToMain({ type: 'progress', payload: { uploadedCount: totalCount, totalCount, percentage: 98, message: 'Видалення старих записів цін...' } });
    ...
    postMessageToMain({ type: 'error', payload: { message: error.message, uploadedCount, totalCount, percentage: Math.min(95, (uploadedCount / totalCount) * 90 + 5) } });
  }
}
