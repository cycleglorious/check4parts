...
<div class="border-primary-950 overflow-hidden rounded-xl border-2">
  <div class="p-4 border-primary-950">
    <div class="grid grid-cols-2 gap-4">
      <div>
        <h3 class="text-lg font-bold text-primary-950">Деталі імпорту прайсу</h3>
        <div class="mt-2 space-y-1">
          <p class="text-sm">
            <span class="font-medium text-primary-900">ID імпорту:</span> 
            <span class="font-mono">{price_history.id}</span>
          </p>
          <p class="text-sm">
            <span class="font-medium text-primary-900">Дата:</span> 
          ...
          <p class="text-sm">
            <span class="font-medium text-primary-900">Постачальник:</span> 
            {price_history.providers?.name || 'Не вказано'}
            {#if price_history.providers?.short_name}
              <span class="text-xs ml-1">({price_history.providers.short_name})</span>
            {/if}
          </p>
          {#if price_history.status === 'actual'}
            <p class="text-sm">
              <span class="font-medium text-primary-900">Статус:</span> 
              <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">
                Актуальний
              </span>
            </p>
            <p class="text-sm">
              <span class="font-medium text-primary-900">Кількість:</span> 
          ...
          {:else if price_history.status === 'deleted'}
            ...
            <p class="text-sm">
              <span class="font-medium text-primary-900">Chunk ID:</span> 
          ...
          {#each warehouses as warehouse}
            <th class="p-2">
              ...
              {#if warehouse.short_name}
                <span class="text-xs  ml-1">({warehouse.short_name})</span>
              {/if}
            </th>
          {/each}
        </tr>
