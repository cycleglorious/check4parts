<script lang="ts">
	...
	let count = $state(0); // кількість рядків у файлі

	let files = $state<FileList>();
	let startFrom = $state(2); // типово з другого рядка
...
<form
	onsubmit={async (e) => {
		...
		await processFile(files!, startFrom, 5, 0, {
...
>
	...
	<div class="form-group">
		<label for="startFromInput">Почати з рядка</label>
		<input
			id="startFromInput"
			type="number"
			min="1"
			...
			disabled={!!template}
		/>
		<p class="form-hint">Вкажіть номер рядка, з якого почнеться обробка</p>
	</div>
...
{#if previewData && template && previewData.length > 0}
	<section class="card" aria-label="Попередній перегляд даних">
		...
		{#if count > 0}
			<p class="form-hint">
				Показано {previewData.length} з {count} рядків (Починаючи з {startFrom})
			</p>
		{:else}
